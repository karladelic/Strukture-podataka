...
int findCity(pozicija head, int number)
{
	...
	while (temp)
	{
		if (temp->populacija >= number)
		{
			printf("Ime: %s  Populacija: %d\n", temp->ime, temp->populacija);
		}
		temp = temp->next;
	}
...
position findCountry(position current, char* name, int number)
{
	...
	{
		printf("Gradovi u %s koji imaju vecu populaciju nego % d:\n", name, number);
...
int readFile(char* name, pozicija head)
{
	...
	while (!feof(pfile))
	{
		...
		fgets(buffer, MAX_SIZE, pfile);
		if (sscanf(buffer, " %s %d", new->ime, &new->populacija) == 2)
		{
...
int compareCities(pozicija new, pozicija old)
{
	int result = new->populacija - old->populacija;

	if (new->populacija == old->populacija)
	{
